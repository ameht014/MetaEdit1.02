## Download all metagenomics samples


SAMPLES_DIR="/home/vsteb002/samples/iHMP/IBD/RNA/"
mkdir -p $SAMPLES_DIR
...
do
 if [[ $string == *"_P"* ]]; then
   wget https://downloads.hmpdacc.org/ihmp/ibd/transcriptome/microbiome/raw/${sample_id}.fastq.gz -P $SAMPLES_DIR
 else
   wget https://downloads.hmpdacc.org/ihmp/ibd/transcriptome/microbiome/raw/${sample_id}.tar -P $SAMPLES_DIR
 fi
...
do
    ...
    cd ${SAMPLES_DIR}/${sample_id} && tar -xvf ${SAMPLES_DIR}/${sample_id}/${sample_id}.tar # && rm ${SAMPLES_DIR}/${sample_id}/${sample_id}.tar
