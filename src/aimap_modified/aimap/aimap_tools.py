...
def construct_bowtie2_index_cmdline(fname):
    cmd = f"bowtie2-build {fname} {fname.replace('.fna', '').replace('.fasta', '')}"
    return cmd

def exec_bowtie2(genome,fqfile1,fqfile2,outdir,outname,threads):
    # bowtie2 -x $INDEX_PATH -1 $READ1 -2 $READ2 -S $OUTPUT
    # args.genomename, new_file1,new_file2,args.outdirname,args.outfile_name,args.threads
    cmd = f"bowtie2 -p {threads} -x {genome} -1 {fqfile1} -2 {fqfile2} -S {outdir}/{outname}.sam"
    return cmd

def exec_bowtie2_single(genome,fqfile,outdir,outname,threads):
    cmd = f"bowtie2 -p {threads} -x {genome} -U {fqfile} -S {outdir}/{outname}.sam"
    return cmd
